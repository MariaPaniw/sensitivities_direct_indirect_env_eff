...
S.j <- function(species,env,N1,N2){
  ...
  if(species==1)
    surv <- inv.logit(3.0 + 0.8*env - 0.05*N1 - 0.03*N2 - 0.03*env*N1)
  
  if(species==2)
    surv <- inv.logit(3.5 + 0.9*env - 0.08*N2 - 0.04*N1 - 0.02*env*N2)
...
T.j <- function(species,env,N1,N2){
  ...
  if(species==1)
    ...
    trans <- inv.logit(0.5 + 0.01*env - 0.15*N1 - 0.01*N2)
  
  if(species==2)
    
    trans <- inv.logit(1 + 0.03*env - 0.1*N2 - 0.012*N1)
